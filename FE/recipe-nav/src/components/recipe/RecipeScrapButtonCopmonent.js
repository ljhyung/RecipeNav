...
const RecipeScrapButtonCopmonent = (props)=>{
    ...
    const scrapButtonClickHadler = ( e)=>{
        ...
        if(isAleardy){
            //이미 찜한거 였다면
            apiClient.delete(`/my-infos/recipes/${recipeRec}`,{
                headers:{
                ...
                message.info('찜해제 완료');
                apiClient.get("/my-infos/recipes",{
                    headers: {
                      Authorization:accessToken
                    },
                  })
            ...
            //찜한게 아니라 이제 찜하는 거라면
            apiClient.post(`/my-infos/recipes/${recipeRec}`,{},
            {
                headers:{
                    Authorization:accessToken
                }
    ...
    return <>
        <button className={style["recipe_scrap_btton"]} onClick={scrapButtonClickHadler}
